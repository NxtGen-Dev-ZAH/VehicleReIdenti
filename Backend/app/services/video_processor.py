...
def analyze_video(db: Session, job: VideoJobORM) -> VideoResult:
    ...
    try:
        result = runner.run(video_path=video_path, artifacts_dir=artifact_dir, job_id=job.id)
    ...
    log_job_event(job.id, "phase_started", phase="persist_result")
    ...
    db.commit()
    db.refresh(job)
    log_job_event(job.id, "phase_completed", phase="persist_result")
    log_job_event(job.id, "job_completed", duration_ms=duration_ms)
